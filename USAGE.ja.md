...
LLMを設定すると、生成フォームに2つのボタンが増えます。

- **「H3用に変換」ボタン**（T2V/I2V/R2V共通）: プロンプト欄に日本語や口語の
  ラフな指示を書いて押すだけで、H3向けの英語プロンプト形式に自動変換されます。
  英語のプロンプト作法を覚えていなくても大丈夫です
- **「🪄 シーンJSONを自動生成」ボタン**（R2V専用）: 【ストーリー】欄に
  日本語で「全体の流れ・起きること」を自由に書き、【リファレンス】欄で
  登場人物を参照ファイルに紐付けておくと、LLMが自動でカットを割り、
  シーンごとのプロンプトを組んだJSONを一括生成してR2Vのプロンプト欄に
  そのまま入れてくれます。カット数・合計秒数は空欄なら自動判定、指定も可能

  つまり「1カットずつ手作業でプロンプトを書く」のではなく、**日本語の演出
  指示（ストーリー）を渡すだけで、複数カットにまたがる連続した動画の
  プロンプト一式を自動生成**できます。長尺のMVやストーリー仕立ての動画を
  作りたい時に特に便利です。
